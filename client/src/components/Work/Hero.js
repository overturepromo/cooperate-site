import React from 'react';

import '../../styles/Work.css'

const Hero = () => {
    return (
        <div className="hero-work">
            <div className="work-hero-box container">
                <p className="work-cta-top animated slideInUp">OUR WORK</p>
                <p className="work-cta-bottom animated slideInUp">Our unconventional approach and full suite of in-house services allow us to give you a personalized promotional marketing program that is effortless, efficient and cost-effective. It's all brought to you with stellar customer service from people who love what they do.</p>
            </div>
        </div>
    )
}


export default Hero;